...
**硬约束 6（数据层视角隔离的强等价性）**：视角隔离必须在数据层完成,且只在事件行粒度上完成——viewer 字段决定该事件能否被某 viewer 取出,payload 内部不得再携带"对部分 viewer 不可见"的子字段。不允许"事件元数据公开 + 部分载荷私有"的混合事件,不允许依赖 prompt 拼装层"兜底"或"修正"过滤 payload。如果业务上确实存在"一部分信息要公开 + 另一部分要私有"的需求,必须拆成两条独立事件(可通过 parent_event_id 关联)。原因:任何绕过 prompt 拼装层的读取路径(quote() / DB Browser / 调试 dump)都会让 payload 私有字段立刻泄露。
...
**三层关系**：数据层是真相源（append-only 不可变），协议层是 agent 行为规约（每次输出怎么写），评估层是兜底监控（崩了能发现）。
**三层缺一不可，且必须按从下到上的顺序实现**——数据层不稳，协议层无意义；协议层不严，评估层只能事后报警。
...
3. **公开发言原文永不压缩**（包括 agent 自己的所有公开发言）。允许压缩的只有"自己的私密推理 / 对手画像"等私域内容，且压缩必须保留可展开回原文的指针。注：自我发言累计量在博弈周期内远小于 prompt context 上限，prompt 拼装层可全量直接注入，无需走指针——但这是实现指导，不构成额外不变量。
4. **摘要者 ≠ 行动者**。让参赛 agent 自己摘要历史是博弈攻击面。
5. **视角隔离在数据层完成**——agent 永远不接触不该看到的事件。具体过滤机制（关系图遍历 / 反规范化字段 / 角色权限等）由实施层决定，但过滤的触发点必须在存储层而非 prompt 拼装层、更不能依赖 agent 自觉。见硬约束 5、6——viewer 字符串封闭集合，禁止 `self` 入库；payload 字段层不承担可见性过滤职责（需要时拆事件）。
6. **回合制 + 单进程串行写入**。博弈天然回合制，无真并发需求，不引入并发协调机制。单进程内即便是 LLM 并行调用的场景，写入 EventStore 也必须串行（互斥锁或单 writer 队列）。多线程并行写入共享 `last_seq` 计数器会破坏 seq 单调性与哈希链顺序。
7. **哈希链 canonical JSON 必须 deterministic**。`canonical_json(payload)` 的实现必须满足：字段按 ASCII 升序排序；数组保留写入顺序；空字符串 vs 缺字段必须可区分。任一规则破坏会导致同一事件序列化两次哈希不同，哈希链审计沦为噪声。实施层必须提供单元测试验证 canonical JSON 的不变性。

### 协议层不变量

8. **每拍 agent 认知输出强制四通道**：私有推理（scratchpad）/ 想说的话（intended）/ 发言意愿打分（bid）/ 给未来自己的便条（note_to_self）。MVP 阶段默认所有参赛 LLM 均支持原生 structured output，Reasoner 必须直接输出符合契约 schema 的四通道 JSON；系统侧由确定性 Validator 校验 schema、字段、枚举、可见性展开、`intended_action` 合法性与事件拆分。LLM 只能直接产出四通道结构化 JSON；`speech.public`、`action.intent` 等可入事件流的外部事件，均由 orchestrator / 执行系统在 Validator 通过后派生，LLM 不得直接声明其最终事件形态、公开落地形态或可见性结果。
...
10. **Reasoner + Validator 架构**。Reasoner LLM（参赛模型，可变）负责生成结构化四通道 JSON；Validator Layer（确定性代码，非 LLM）负责校验 JSON schema、必填字段、枚举、bid 范围、visibility 展开、`intended_action` 合法性与 canonical JSON。校验失败必须 reject sample 重试 Reasoner；多次失败后写入 `system.validation_failed` 事件并令该拍该 agent abstain。MVP 主路径不引入独立解析模型。
...
| **输出契约版本 + Validator 版本** | structured output schema、Validator 校验规则、枚举词汇表与事件拆分规则的版本号。任一变更必须递增版本；事后审计代码可按版本号选择对应校验逻辑回放。MVP 阶段固定为 `"1"`。                                                                                                                                                                       |
| **原始 LLM 输出**                 | Reasoner LLM 的完整响应（包括结构化输出原文），调试、校验失败定位与审计用                                                                                                                                                                                                                                                                      |
...
- 每拍 agent 认知输出的四通道（scratchpad / intended / bid / note_to_self）作为四条独立事件写入，共享同一父事件引用，但**可见性不同**。`speech.public` 作为衍生事件写入时，必须以源 `speech.intended` 的 event_id 作为 `parent_event_id`。bid 事件的 `visibility = ["orchestrator"]`（agent 互相看不到对方 bid，避免伪礼让）。`intended_action` 及其派生事件的生命周期与可见性见 §5.2.4。
...
| 必保留 3 | **自我发言全量原文**          | 自己从开局到现在所有公开发言 + 私聊                                        | 按 actor=<current_agent_id> 过滤事件流 | 否         |
| 必保留 4 | **我最近想说但没说出口的话**  | 自己最近 K 拍写过 speech.intended 但未抢中 floor 的内容（含建议性 @ 目标） | pending_intended 投影                  | 否         |
| 必保留 5 | 自己的便条历史                | 每拍的"给未来自己的便条"累积，tick-level 反思载体                          | 事件流                                 | 否         |
| 必保留 6 | 自我承诺投影                  | 代码抽取的承诺 / 声明 / 投票表                                             | commitments 投影                       | 否         |
| 必保留 7 | 公开发言近场窗口              | 最近 K 拍所有当前 agent 可见的事件原文                                     | 事件流按拍次/轮次过滤                  | 否         |
...
**search_history 返回规范**：`search_history` 只返回事件指针与必要的结构化元数据，不返回 free-form summary。完整原文必须通过 `quote(seq, viewer)` 获取。

返回条目至少应包含可确定定位事件的字段，例如：
...
### 5.1 每拍认知输出协议(scratchpad / intended / bid / note_to_self)

每拍每个「在场」agent 必须使用模型原生 structured output，直接输出一个严格 JSON 对象。
...
本节描述的是 LLM 直接产出的内容；`speech.public` 不在 LLM 输出中——它是 orchestrator 在 bid 裁决并通过 Listener-as-filter 后由源 `intended` 衍生的事件。
````

**Validator 硬规则**：

- 顶层必须且只能包含 `scratchpad` / `intended` / `bid` / `note_to_self` 四个对象。
- `scratchpad.text` 是私有推理，事件类型 = `speech.scratchpad`，可见性 = `[<actor_id>]`；它是 tick-local 草稿，下一拍不再注入 prompt。
- `intended.text` 是「我这一拍想说的话」，事件类型 = `speech.intended`，可见性 = `[<actor_id>]`；即便本拍未抢中 floor、未衍生 public，也永久留存。
- `intended.intended_action` 可省略或为 `null`；若存在，必须恰好包含一个合法 intent，且参数满足 §5.2 的 intent ontology。
- `bid.urgency_level` 必须是封闭枚举值之一：`pass` / `low` / `medium` / `high` / `urgent` / `critical`。LLM 不得直接输出数值 urgency。
- Validator / orchestrator 必须将 `urgency_level` 确定性映射为内部 `urgency_score`，用于 floor control 排序。`urgency_score` 是系统派生字段，不属于 Reasoner LLM 可写字段。
- `bid` 事件可见性 = `["orchestrator"]`，agent 互相看不到对方 bid，避免伪礼让。
- `note_to_self.text` 是给未来自己的便条，事件类型 = `speech.note`，可见性 = `[<actor_id>]`，下一拍自动注入自己的 prompt。
- 所有 `visibility` 在入库前必须由系统展开为封闭 viewer 集合；禁止 `self` 进入持久层。
- 任一字段缺失、类型错误、enum 未命中、额外顶层字段、非法 viewer、非法 action intent 均视为 validation failed，触发 reject sample 重试 Reasoner。

**写入规则**：四通道共享同一父事件引用，但分别写为四条事件，**可见性不同**。**`speech.public` 不在 agent 输出里**——它是 orchestrator 在以下流程后产生的衍生事件：

1. 收齐所有在场 agent 本拍的 structured output，并全部通过 Validator。
2. 按 `urgency + bid_offset` 选最高者（详见 §5.2bis）。
3. 取该 agent 的 `intended.text`，过 Listener-as-filter（§5.3）做泄露审查。
...
本节只定义发言通道的写入与 `speech.public` 衍生流程。`intended.intended_action` 的派生、生命周期和可见性见 §5.2.4。Listener-as-filter 只对将要衍生为 public 的 intended 文本生效；动作不参与 listener 审查，因为动作语义不通过公开文字传播。
...
每个在场 agent 本拍**必出** `bid` 对象。bid 事件 `visibility = ["orchestrator"]`，其他 agent 通过 `quote(seq, "NPCxx")` 永远拿到不可见标记。这是核心约束——**agent 互相不能看到 bid，否则会出现"我看你想说我就不抢"的伪礼让**，破坏博弈真实性。

bid 字段含义：

| 字段              | 类型   | 说明                                                                                      |
| ----------------- | ------ | ----------------------------------------------------------------------------------------- |
| `urgency_level`   | enum   | `pass` / `low` / `medium` / `high` / `urgent` / `critical`，表示"我多想说这一拍"          |
| `urgency_score`   | float  | 系统派生字段，LLM 不得输出；由 Validator / orchestrator 从 `urgency_level` 确定性映射得到 |
...
收齐所有在场 agent 的 bid 后，Validator 先将 `urgency_level` 映射为内部 `urgency_score`，再按 `urgency_score + bid_offset` 降序排：

1. **抢中规则**：最高者抢中 floor，其 intended 走 Listener-as-filter 后衍生 speech.public。
2. **冷场规则**：最高 `urgency_score` < 阈值（建议 3.0），等价于最高档位不超过 `low` → 本拍无人发言，orchestrator 触发场景推进（直接提问、阶段切换、引入环境事件）。`tick_resolved` 事件 `winner_actor=null`。这避免冷场死锁。
...
- intent 必须从词汇表中选择；未识别 intent 由 Validator 拒绝并 reject sample 重试 Reasoner
...
- `intended.intended_action` 是私有动作意图；其他 agent 不能看到意图原文，只可能通过感知系统看到执行过程与结果（`action.resolved` / `action.cancelled`）。这些结果事件的可见性由执行系统结合感知系统填入，不由 LLM 决定。
...
{
  "scratchpad": {
    "text": "NPC05 上拍的反应模式像狼，confidence 0.7。但现在指认会暴露我是 Seer。"
  },
  "intended": {
    "text": "我觉得今天大家都聊得很谨慎，我没什么具体怀疑对象。"
  },
  "bid": {
    "urgency": 4.5,
    "rationale": "可以说但不是必须，让 NPC07 先动"
  },
  "note_to_self": {
    "text": "下拍如果 NPC05 主动跳 Seer，就反 Seer 暴露他。"
  }
}
```
...
### 5.4 Reasoner + Validator 架构

MVP 阶段不引入独立解析模型。所有参赛 LLM 必须使用原生 structured output，直接产出符合协议 schema 的四通道 JSON。系统侧只保留确定性 Validator Layer，负责校验、拒采样、事件拆分和审计版本化。

```
┌──────────────┐  structured JSON  ┌──────────────────┐  validated intended  ┌──────────────┐
│  Reasoner    │ ────────────────> │    Validator     │ ───────────────────> │  Listener    │
│  LLM (参赛)  │                   │  deterministic   │                      │   filter     │
└──────────────┘                   └────────┬─────────┘                      └──────┬───────┘
                                             │                                       │
                                             v validation failed                     v leakage risk
...
**Reasoner LLM**：参赛模型（Claude / GPT / Gemini / GLM / Qwen / DeepSeek 等），可变。负责直接生成 structured JSON（含四通道：scratchpad / intended / bid / note_to_self）。

**Validator Layer**：确定性代码，非 LLM。它不做语义补全，不改写 agent 内容，只负责把结构化输出判定为「可入库」或「不可入库」。职责包括：

1. 校验 JSON schema 与顶层四通道字段；
2. 校验字段类型、必填字段、字符串长度、数值范围；
3. 校验 `bid.urgency` ∈ `[0.0, 10.0]`；
4. 校验 `intended.intended_action` 最多一个 intent，且 intent / params 命中词汇表；
5. 校验 `addressed_to_hint`、`proposed_target`、`relates_to_seq` 等引用字段格式；
6. 展开 visibility：把 prompt 层相对语义转换为具体 viewer，禁止 `self` 入库；
7. 将四通道拆分为独立事件，并附加 `output_contract_version` / `validator_version`；
8. 保存 Reasoner 完整响应与结构化输出，供调试、审计和失败复盘；
9. 校验 canonical JSON 可稳定序列化；
10. 校验失败时 reject sample 重试 Reasoner。

**失败处理**：validation failed 后重试 Reasoner 最多 3 次；仍失败则写 `system.validation_failed` 事件，保留原始响应和失败原因，该拍该 agent 视为 abstain（bid=0）。失败本身必须进入 append-only 事件流。

**边界声明**：本契约的 MVP 主路径不定义独立解析模型。若未来要兼容不支持 structured output 的模型、处理历史非结构化日志、或对非结构化文本做迁移导入，可以在实施层新增「legacy adapter」；该 adapter 不属于协议主路径，且不得绕过 Validator。

### 5.5 三级反思层级

| 级别            | 周期                 | 内容                                                                 | 谁产生                 | 写入位置（语义层）                   |
| --------------- | -------------------- | -------------------------------------------------------------------- | ---------------------- | ------------------------------------ |
| **tick-level**  | 每拍 agent 输出后    | 给下拍自己的便条                                                     | agent 自己（参赛 LLM） | 便条事件，可见性 = [<actor_id>]      |
| **round-level** | 每轮结束             | Reflection 9 问的回答（私有）                                        | agent 自己（参赛 LLM） | 反思事件，可见性 = [<actor_id>]      |
...
每轮结束时强制让每个 agent 回答以下 9 个问题（私有，作为反思事件写入，可见性 = [<actor_id>]）：
...
**实现细节**：用 structured output 强制 JSON 格式，写入时由 Validator 校验 9 项是否齐全、类型是否正确、评分是否在范围内。第 5、6 项的可疑度/可信度评分直接喂给 commitments 投影作为"我对每个对手的怀疑度"投影段。
...
| Structured output 校验失败           | Validator 记录失败原因并 reject sample 重试 Reasoner 最多 3 次；3 次后写 `system.validation_failed` 事件；保留原始 LLM 输出和结构化输出供事后人工审计；该拍该 agent 视为 abstain（bid=0）              |
...
commitments projector 不仅扫 speech.public，也扫**未抢中 floor 的 speech.intended**。原因：agent 即便没抢到话语权，它在 intended 里写「我打算指控 NPC05」也已经构成内部承诺——后续 round-level 反思 q7「我已暴露什么」可以审计：虽然没说出口，但你是这么打算的。这个扩展只对 agent 自己可见（intended 的 visibility=[<actor_id>]），不会暴露给对手；但用于自我连续性追踪和事后行为审计。
...
>
> append-only 事件流加结构化字段过滤，其失败模式是**已知的运维问题**；四通道 structured output + Validator + bid 协议，其失败模式是**已知的 schema 契约、校验与裁决公平性问题**；BEL_EXT 9 项 + GOAL/BEL 双监控，其失败模式是**已知的评估器选型问题**。其他所有方案的失败模式（向量召回错失、LLM 摘要改写、context rot、agent 自爆、长程退化、齐发模型导致的"被打断"语义裂缝）在对抗博弈目标场景下都是**未量化的认知问题**。
...
scratchpad.text:
- Think step-by-step about what just happened, what each opponent might be,
  what you must NOT say publicly, what you would say if you got the floor,
  whether speaking this tick is worth it, and what action (if any) advances your goal.
- This field is private and one-shot. It will NOT be injected into your prompt next tick.

intended.text:
- What you WOULD say this tick if you win the floor.
- Important: you may not get the floor. The orchestrator picks ONE speaker per tick by bid.
  If another agent wins, your intended text is preserved verbatim in your event history
  and will appear next tick under "what I wanted to say but didn't get to."
- Even if you do NOT want to speak, write "(passing)" or "" and use a low bid.
- Do not reveal your role, faction, private goals, private chat, or private reasoning here.
  Listener-as-filter will run on this text BEFORE it becomes public.

intended.intended_action:
- Optional. Use null if you do not want to act this tick.
- When present, it must be exactly one object: {"intent": "<name>", "params": {...}}.
- Unknown intent names or invalid params will fail validation.
...
{"intended": {"text": "...", "intended_action": {"intent": "<name>", "params": {<json>}}}}
