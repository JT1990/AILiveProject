...
## 目标
实现骗子酒馆的 3 个专属 Action 类。**遵循 T11 的 Validate/Apply 拆分**：Action 本身不修改游戏状态——状态修改由 GM.Apply 完成；Action 只做"参数解析 + 触发 TTS/动画 + 调 GM.RecordSpeechEvent + Done(true) 让 MindComponent 通知 GM 去 Apply"。

## 前置
T12（GM 阶段机能跑）+ T11（DispatchAction → Validate → Execute → Apply 链路）+ GM.RecordSpeechEvent 实现

## DoD
- [ ] `UMindAction_PlayCards::Execute`：
  - 解析 `{claim_rank: "K"|"Q"|"A"|"Joker", claim_count: 1-3, actual_indices: int[]}`
  - 触发 Speak TTS（fire-and-forget）配音"我打了 X 张 Y"
  - 同时调 `GM.RecordSpeechEvent(self, "public", line, listeners=AllPlayersInHearing)` push `[公开]` 到所有 listener.Context（**不调 MemoryClient.Write**——event 由 HandleActionDone 统一写一次）
  - 立即 `Done(true, "claimed N R")`
- [ ] `UMindAction_Challenge::Execute`：
  - 触发 TTS"我质疑！" + GM.RecordSpeechEvent(public, listeners=All)
  - `Done(true, "challenge")`
- [ ] `UMindAction_PassTurn::Execute`：
  - 触发 TTS"我跳过这轮" + GM.RecordSpeechEvent(public, listeners=All)
  - `Done(true, "pass")`
- [ ] **schema 在 ParamSchemaJson 字段中暴露**（构造函数里设；由 ContextManager.UpdateLayer0bFrame 拼到 messages[1] 的 `== 可用动作 ==` 段）
- [ ] **状态修改路径**（按 T11 流程）：
  ```
  Dispatch(env={action:play_cards,...})
    ├─ GM.Validate: 检查 indices 在 hand 范围、count 一致 → 通过
    ├─ Action.Execute: 触发 TTS + GM.RecordSpeechEvent + Done(true)
    ├─ HandleActionDone:
    │   ├─ GM.Apply: 真正从 hand 移除 actual_indices 的牌、记 claim
    │   ├─ MemoryClient.WriteEvent(...)              # 一次 fire-and-forget event log
    │   └─ Context.PushUser("[结果] play_cards: ...")
...
void UMindAction_PlayCards::Execute(UMindComponent* Owner, const FString& ParamsJson, FOnActionDone Done) {
    ...
    // 1. TTS 配音 (用 ResolveSpeechActor 拿可见 child actor)
    ...
    // 2. 通知 GM 推送到 listener Context (不调 MemoryClient.Write)
    if (auto* GM = Owner->GetGameMaster()) {
        TArray<AActor*> Listeners = GM->GetAllActiveAgents();   // 公开发言 listeners=全员
        GM->RecordSpeechEvent(Owner->GetOwner(), TEXT("public"), Line, Listeners);
    }
...
void UMindAction_Challenge::Execute(UMindComponent* Owner, const FString& ParamsJson, FOnActionDone Done) {
    ...
    UMinimaxACELibrary::TriggerMinimaxSpeech(
        ...
        Owner->Config->A2FProviderName);
    if (auto* GM = Owner->GetGameMaster()) {
        GM->RecordSpeechEvent(Owner->GetOwner(), TEXT("public"), Line, GM->GetAllActiveAgents());
    }
...
// Execute 类似（短 Speak + RecordSpeechEvent）
```

## 验收信号

执行 T12 验收的同时：
- LLM 决策返回 `play_cards` 时 → Validate 通过 → Action.Execute 触发 NPC 喊"我打了 X 张 Y" + 口型同步 → GM.RecordSpeechEvent push `[公开] NPC_X 说: ...` 到所有 listener Context → Apply 修改 hand → MemoryClient.WriteEvent 写 event log
- 后续 ChallengeWindow 阶段，某个 NPC 选择 challenge → 喊"我质疑！" → push `[公开]` → GM 切 Reveal
- Output Log 按 `LogMindAction` 过滤能看到 `play_cards: claimed 2 K`、`challenge: from npc_3`
- Memory Service `/memory/event` 中能查到对应 event 记录（每个 Speak 一条 event，不是每个 listener 一条）
- listener 的 `Context.Layer2` 中 `[公开]` 消息出现（用 LogMind Verbose 抓切片验证）

## 不在范围
- 出牌动画 / 拿枪动画（T15）
- claim 与 actual 不一致时的"被拆穿后表演"（T15）
- 多 challenge 竞争优胜机制的复杂逻辑（T12 已是先到先得）
- 私聊 channel（T22 加 Speak 的 channel 参数支持 public/private）
...
- **GM.Apply 是同步的，但 TTS 是异步的——表演会在 hand 实际改动之后几百毫秒才发声**：观众视角看是先静默改 HUD（discardpile +1）再听到 NPC 喊。可接受，M3 polish 时通过 Montage 同步表演（T15）
- RecordSpeechEvent 的 listeners 列表由 GM 提供（公开发言 = 全员，私聊 = T22 才支持 channel 参数）
