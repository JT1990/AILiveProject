# Mesh Action Reference

Full action signature tables for `mesh_query()`. Read this file when you need exact parameter names, key params, or need to verify what a specific action does.

## Table of Contents
1. [New in Overhaul](#new-in-overhaul)
2. [Mesh Inspection (12)](#mesh-inspection-12-actions)
3. [Scene Manipulation (8)](#scene-manipulation-8-actions)
4. [Scene Spatial Queries (11)](#scene-spatial-queries-11-actions)
5. [Level Blockout (15)](#level-blockout-15-actions)
6. [Mesh Operations (12)](#mesh-operations-12-actions)
7. [Horror Spatial Analysis (8)](#horror-spatial-analysis-8-actions)
8. [Accessibility Analysis (6)](#accessibility-analysis-6-actions)
9. [Lighting Analysis (5)](#lighting-analysis-5-actions)
10. [Audio & Acoustics (14)](#audio--acoustics-14-actions)
11. [Performance Analysis (5)](#performance-analysis-5-actions)
12. [Decal & Detail Placement (4)](#decal--detail-placement-4-actions)
13. [Level Design (9)](#level-design-9-actions)
14. [Volumes & Properties (7)](#volumes--properties-7-actions)
15. [Horror Intelligence (4)](#horror-intelligence-4-actions)
16. [Tech Art Pipeline (7)](#tech-art-pipeline-7-actions)
17. [Advanced Level Design (8)](#advanced-level-design-8-actions)
18. [Context-Aware Props (8)](#context-aware-props-8-actions)
19. [Procedural Geometry (8)](#procedural-geometry-8-actions)
20. [Procedural Town Generation (46)](#procedural-town-generation-46-actions)
21. [Genre Presets (8)](#genre-presets-8-actions)
22. [Encounter Design (8)](#encounter-design-8-actions)
23. [Quality & Polish (9)](#quality--polish-9-actions)
24. [Blockout Tag Convention](#blockout-tag-convention)
...
## Mesh Inspection (12 actions)
Read-only, query any StaticMesh/SkeletalMesh.
...
## Scene Manipulation (8 actions)
Actor CRUD on placed level actors.
...
## Scene Spatial Queries (11 actions)
Physics-based world queries.
...
## Level Blockout (15 actions)
Tag-based blockout volumes, asset matching, replacement.
...
## Mesh Operations (12 actions)
`#if WITH_GEOMETRYSCRIPT` — require handles.
...
*Work-in-progress: functional but still being refined.*
...
## Accessibility Analysis (6 actions)
...
## Audio & Acoustics (14 actions)
Physical material-aware spatial audio. *Work-in-progress: functional but still being refined.*
...
## Performance Analysis (5 actions)
Budget-aware placement. *Work-in-progress: functional but still being refined.*
...
## Decal & Detail Placement (4 actions)
...
## Level Design (9 actions)
Lights, materials, mesh swap, instancing.
...
## Volumes & Properties (7 actions)
Trigger, kill, navmesh, selection.
...
## Horror Intelligence (4 actions)
Player path prediction, encounter scoring.
...
## Tech Art Pipeline (7 actions)
Import, LOD gen, texel density, collision.
...
## Advanced Level Design (8 actions)
Sublevels, prefabs, splines.
...
## Context-Aware Props (8 actions)
Surface scatter, disturbance, physics.
...
## Procedural Geometry (8 actions)
GeometryScript, requires handle pool.
...
## Procedural Town Generation (46 actions)
Full building-to-block pipeline. Generates entire city blocks from archetype definitions through a layered pipeline: floor plans -> grid geometry -> facades -> roofs -> streets -> spatial registry -> volumes -> furnishing.

Requires `bEnableProceduralTownGen = true` in Editor Preferences (disabled by default).

### SP1: Grid Building (2)
| Action | Key Params | Purpose |
|--------|-----------|---------|
| `create_building_from_grid` | `grid`, `descriptor`?, `omit_exterior_walls`? | Grid -> geometry + Building Descriptor output |
| `create_grid_from_rooms` | `rooms` | Room rects -> occupancy grid |

### SP2: Floor Plan Generator (3)
| Action | Key Params | Purpose |
|--------|-----------|---------|
| `generate_floor_plan` | `archetype`, `footprint`, `floor_index`? | Archetype + footprint -> grid/rooms/doors |
...
### SP3: Facade & Window Generation (3)
| Action | Key Params | Purpose |
|--------|-----------|---------|
...
### SP8b: Architectural Features (5)
| Action | Key Params | Purpose |
|--------|-----------|---------|
| `create_balcony` | `location`, `size`?, `railing_style`?, `building_context`? | Floor slab + railing; emits `wall_openings` |
| `create_porch` | `location`, `size`?, `columns`?, `building_context`? | Covered entry with columns; emits `wall_openings` |
| `create_fire_escape` | `building_id`?, `floors`?, `building_context`? | Zigzag exterior stairs; emits `wall_openings` |
| `create_ramp_connector` | `start`, `end`, `width`?, `building_context`? | ADA-compliant ramp; emits `wall_openings` |
| `create_railing` | `path_points`, `style`?, `building_context`? | Railing along arbitrary path; emits `wall_openings` |

### SP9: Daredevil Debug View (6)
| Action | Key Params | Purpose |
|--------|-----------|---------|
...
| Action | Key Params | Purpose |
|--------|-----------|---------|
| `validate_building` | `building_id` | Capsule sweep walkability, BFS connectivity, stair angles, window raycasts |

---

## Genre Presets (8 actions)
...
## Encounter Design (8 actions)
Horror advanced + accessibility.
...
Standard actor tags (`TArray<FName>`) on `ABlockingVolume` — zero runtime footprint:
