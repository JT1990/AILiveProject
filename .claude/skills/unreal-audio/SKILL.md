...
## Known Quirks (2026-04-25 Test Pass)

### `search_audio_assets` returns duplicates (issue #5)

Result list is NOT de-duplicated. Identical asset paths can appear multiple times.
**Fix:** post-filter unique by `asset_path`, or prefer `list_audio_assets` + client-side substring match for clean enumeration. Counts (`total`, `count`) reflect the duplicated array — do NOT use them for distinct-asset counts.

### `add_metasound_interface` has no discovery action (issue #8)

Use this canonical list — verified registered in UE 5.7:
...
| `UE.Attenuation` | Adds Distance + WorldPosition system inputs |
| `UE.Spatialization` | Adds Azimuth/Elevation system inputs |

Required combinations: Source assets need at minimum `UE.Source` + ONE `UE.OutputFormat.*`. For one-shot SFX: `UE.Source.OneShot` + `UE.OutputFormat.Mono`.

### Pin-name discoverability (issue #9)

**Sound Cue nodes** — use `connect_sound_cue_nodes` with `child_index` (integer, NOT pin name):
...
| `WavePlayer`, `Dialogue Player` | terminal — no children |

Prefer `build_sound_cue_from_spec` — it handles wiring via node IDs and avoids the index trap entirely.

**MetaSound nodes** — pin names ARE discoverable via `find_metasound_node_inputs`/`outputs` on the just-added node. Common verified pins:

| Node class | Inputs | Outputs |
|---|---|---|
| `["UE", "Wave Player", "Mono"]` | `Wave Asset`, `Start Time`, `Pitch Shift`, `Loop`, `Play`, `Stop` | `Audio`, `On Play`, `On Finished` |
| `["UE", "Multiply", "Audio Float"]` | `Audio`, `Float` | `Out` |
| `["UE", "Add", "Audio"]` | `In Audio 0`, `In Audio 1` | `Out Audio` |
| `["UE", "LFO", "LFO"]` | `Frequency`, `Amplitude`, `Offset`, `Type`, `Reset` | `Out` |

### Batch actions require `asset_paths` array (issue #10)

Every `batch_*` action AND `apply_audio_template` takes `asset_paths` (plural, array). Single `asset_path` (string) returns "missing required field". Always wrap: `{"asset_paths": ["/Game/Audio/MyWave"], ...}`.

### Sound Cue node IDs re-index after removal (issue #11)

After `remove_sound_cue_node`, re-call `get_sound_cue_graph` and rebuild the node-ID map — cached IDs are stale. MetaSound uses stable GUID handles and is NOT affected.
