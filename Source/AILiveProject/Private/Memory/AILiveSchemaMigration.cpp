...
bool ApplyMigrationV0ToV1(FSQLiteDatabase& InDb, bool bIsMetaDb, const TCHAR* FtsTokenizer)
{
	...
	if (bIsMetaDb)
	{
		...
		{
			if (!ExecOrFail(InDb, Sql, TEXT("_meta.db DDL"))) { return Rollback(); }
		}
	}
	else
	{
		...
		{
			if (!ExecOrFail(InDb, Sql, TEXT("game.db DDL"))) { return Rollback(); }
		}
		...
		const FString FtsCreateSql = FString::Printf(
			...
			FtsTokenizer);
		if (!ExecOrFail(InDb, *FtsCreateSql, TEXT("events_fts virtual table"))) { return Rollback(); }
		if (!ExecOrFail(InDb, GameDb_TriggerEventsFtsAi, TEXT("events_fts_ai"))) { return Rollback(); }
		if (!ExecOrFail(InDb, GameDb_TriggerEventsFtsAd, TEXT("events_fts_ad"))) { return Rollback(); }
