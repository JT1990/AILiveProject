...
ACT01 是 Zombie Game 规则介绍的开场幕。按 `数字键1` 或 console `act01.start` 触发：10 NPC 拉回各自牢房 → 1s 内打开牢门 → 全员通过 EQS 散点到电视前 → `MediaPlate` 播放 `game-introduction.mp4` → 视频结束完成。

## 触发方式

- 按键：`数字键1`（`StartKey` UPROPERTY 可改）
- Console：`act01.start` / `act01.cancel`（运行时自动注册到 `IConsoleManager`）
...
PlayingVideo (MediaPlate Open 异步 → IsReady → Play → 等 OnEndReached)
  ↓
Idle (Broadcast OnAct01Completed)
```
...
- `Source/AILiveProject/Public/Acts/Act01RuleIntroDirector.h`（新）
- `Source/AILiveProject/Private/Acts/Act01RuleIntroDirector.cpp`（新）
- `Source/AILiveProject/AILiveProject.Build.cs`：新增 `"MediaAssets"` + `"MediaPlate"` 依赖

### 资产

- `Content/Blueprints/Acts/BP_Act01Director`（新）：C++ 类的 placeable wrapper，CDO 持软引用与配置
- `Content/Prison/Blueprints/BP_cell`：`SM_Door.bCanEverAffectNavigation = False`（关键 nav 修复，见踩坑#2）
- `Content/MyAssets/Levels/L_prison.umap`：放 `BP_Act01Director_C_0` 实例，folder `/Acts/Act01`
...
软引用（NavTarget / NavLookTarget / MediaPlate）支持 fallback：`LoadSynchronous` 失败 → `GetAllActorsOfClass` + Label 匹配 → 退化 first instance。BeginPlay 和 BeginAct01 各执行一次。
...
BeginPlay 时枚举 `NPCMoverClass` 实例缓存 `TMap<TWeakObjectPtr<AActor>, FTransform>`。每次按 1 触发先 `SetActorTransform` 拉回 + `AIController::StopMovement`，反复测试 ACT01 不需要 stop/start PIE。
...
`mesh_query::spawn_actor` 只接 StaticMesh；`place_blueprint_actor` 拒绝 `/Script/...` C++ 类路径。绕路：`blueprint_query::create_blueprint` 建空 BP（parent_class=`/Script/AILiveProject.Act01RuleIntroDirector`） → `place_blueprint_actor` 放 BP 实例。
...
`ResolveTargetActors` 加 fallback：解析失败 → `GetAllActorsOfClass(NavTargetClass)` + label 匹配（`NavTargetActorLabel="BP_NavTarget_1"`）→ 仍失败退化 first instance。BeginPlay 和 BeginAct01 各调一次。
...
按 `数字键1` 或 console `act01.start`：

1. 屏幕 `[Act01] reset N NPC(s) and 10 door(s) to initial state`（白）
2. 1s 内 10 扇门 RelativeRotation `(0,0,0)` → `(0,-120,0)` 平滑 lerp，统一向走廊外开
3. NPC 通过 EQS 散点到 NavTarget 附近不同位置，到达后转身朝 BP_NavLookTarget_TV
4. 屏幕 `[Act01] video open requested: <url>`（绿）
5. 视频加载完后屏幕 `[Act01] video started: <url>`
6. 视频播放结束 → `[Act01] video OnEndReached fired` → `[Act01] complete`
...
```
LogAct01: [Act01] BeginPlay cached N NPC(s), 10 cell door(s)
LogAct01: [Act01] resolve: NavTarget=... NavLook=... MediaPlate=...
LogAct01: [Act01] StartKey pressed via PC
LogAct01: [Act01] reset N NPC(s) and 10 door(s) to initial state
LogAct01: [Act01] opening cell doors
LogAct01: [Act01] dispatched N NPC(s) via EQS scatter around NavTarget
LogAILiveScatter: [Scatter] firing EQS '...' with Querier=... for N NPCs (skipped K player-controlled)
LogAct01: [Act01] video open requested: file://...
LogAct01: [Act01] waiting for video open elapsed=1.0s url=...
LogAct01: [Act01] video started: file://...
LogAct01: [Act01] video OnEndReached fired
LogAct01: [Act01] complete
```
...
- ACT02 落地时根据形态决定是否抽 `AActChapterBase`（共用 StartKey 轮询、Reset transforms、ScatterMover 派发、MediaPlate 播放）
