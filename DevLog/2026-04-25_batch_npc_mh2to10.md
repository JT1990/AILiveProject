...
```
1. 复制模式到 BP_MH_Character_2~10 的场景 NPC
2. 解决 T 键按下时 10 个 NPC 同时响应的问题（改 ChildActor BeginPlay 的 EnableInput 策略 或换代码驱动）
3. 可选清理 GM_Sandbox.VisualOverrides[6]
```

## 功能描述

把对 `MH_Character_1` 的 "NPC 壳子 + MetaHuman 视觉 + GASP Mover + A2F" 模式复制到 `MH_Character_2~10`。结果：场景里 10 个 MetaHuman NPC 全部是 `SandboxCharacter_Mover` 子类实例，都带 GASP Motion Matching + Idle 不 T-Pose + A2F 面部组件（代码可驱动任意 NPC 说话）。
...
| 关卡       |    9 | 删 `BP_MH_Character_N_C_1` NPC、原位 spawn `BP_NPC_MH_Character_N`（label `NPC_MH_Character_N`）                    |
...
预检发现 `BP_MH_Character_2~10` **本来就没有 T 键触发链**（EventGraph 里只有 LiveLinkSetup 相关节点，`K2Node_InputKey` 不存在）。A2F/TTS 集成只在 `BP_MH_Character_1` 做过。
...
对每个 `BP_MH_Character_N` (N=2..10)，4 步：
...
对每个 `BP_NPC_MH_Character_N` (N=2..10)，5 步：
...
关卡替换：`delete_actors` 9 个旧实例 + `place_blueprint_actor` 9 个新 NPC，坐标同原位。

### 批量策略

所有 9 个 BP 的操作完全同构，利用 MCP 并行调用：

- **Phase 1**（32 并行）：9 个 BP 的 4 个 `set_component_property`（MH_Character_1 已先做）
- **Phase 2**（8 并行）：9 个 `add_component`（MH_Character_2 上一步就做了）
- **Phase 3**（8 并行）：9 个 `build_blueprint_from_spec`（10 节点 + 12 连线 + 1 pin default）
- **Phase 4**（8 并行）：9 个 `connect_pins`
- **Phase 5**（18 并行）：9 个 `compile_blueprint` + 9 个 `save_asset`
- **Phase 6**（9 并行）：9 个 `create_blueprint`
- **Phase 7**（9 并行）：9 个 `add_variable`
- **Phase 8**（9 并行）：9 个 `set_variable_defaults`
- **Phase 9**（9 并行）：9 个 `build_blueprint_from_spec`（3 节点 + 2 连线）
- **Phase 10**（9 并行）：9 个 `connect_pins`
- **Phase 11**（18 并行）：9 个 compile + 9 个 save
- **Phase 12**（1+9 并行）：`delete_actors` 9 个 + `place_blueprint_actor` 9 个
- **Phase 13**（4 并行）：set_cdo_property + compile GM + save GM + save level

**实际用时**：估计比单个 BP 做 18 次 serial 快 ~10 倍。
...
- **批量并行**。对 9 个同构改动用单轮 MCP 并行批处理，总共 12 轮 × 9 并行 ≈ 100 个 MCP 调用 在 ~2 分钟内完成。比逐个 serial 省 ~10x 时间
- **预检一次解决 3 个任务**。Task #13（T 键多响应）的方案本来规划为"加 Tag 过滤" 或 "改 EnableInput"；预检发现 MH_2~10 根本没 T 键链，自动解决。这个发现直接干掉 Task #13 整个子任务
- **复用上一轮的 id 映射惯例**。BP_MH_Character_2~10 和 BP_MH_Character_1 的 EventGraph BeginPlay 头部 `K2Node_Event_0` 一致，新建的 `K2Node_CallFunction_1`（delay）一致，所以不需要逐个查 graph data，可以批量 `connect_pins(K2Node_Event_0.then → K2Node_CallFunction_1.execute)`
- **精简 BeginPlay 链**。上一轮 MH_Character_1 的 BeginPlay 有 `EnableInput + PrewarmA2F + GetAvailableA2FProviders + AddTickPrerequisite` 14 节点；MH_2~10 只要 10 节点（去掉 input/prewarm）—— NPC 代码驱动，不需要用户输入路由；Prewarm 在 MH_Character_1 已经全局调过了

### 踩过的坑

1. **MH_Character_4 的实例名有 `_C_2` 后缀**（不是 `_C_1`）。批量 delete_actors 时要挨个列出准确名字，不能按 pattern。预检时 `get_level_actors` 查清楚再删
2. **9 个 BP 的构建虽然同构，但仍要 9 次独立 `build_blueprint_from_spec`**。`build_blueprint_from_spec` 不支持一次多个 asset_path，只能并行

### 值得沿用的模式

- **"模板-实例"批量改造**。先做 1 个（MH_Character_1），作为模板验证；再并行复制到 N 个同构目标。省掉重复 debug。上一轮 MH_1 跑通后，这轮 MH_2~10 0 个重新 debug 周期
...
- **场景 NPC 占位 overlap 问题**：10 个 NPC 都是 `SandboxCharacter_Mover` 壳子 Pawn，带 `Capsule` 碰撞体，靠得近的几个（如 MH_Character_3 和 MH_Character_2 相距 400 单位）可能发生碰撞或挤压。如果后期 AI 驱动移动，要注意避让/导航
- **性能**：10 个独立 Pawn + 10 个 ChildActor + 10 个 MetaHuman Mesh (Face/Body/Grooms/Clothing) 运行时开销大。可以先不优化，PIE 下观察 `stat unit` 和 `stat anim`
